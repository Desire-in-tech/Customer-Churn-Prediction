...
import { 
  ...
  usePredictChurn, 
  CustomerInputGender,
  CustomerInputPartner,
  CustomerInputDependents,
  CustomerInputPhoneService,
  CustomerInputMultipleLines,
  CustomerInputInternetService,
  CustomerInputOnlineSecurity,
  CustomerInputOnlineBackup,
...
import { getGetPredictionHistoryQueryKey } from "@workspace/api-client-react";
...
const formSchema = z.object({
  ...
  paymentMethod: z.enum(["Electronic check", "Mailed check", "Bank transfer (automatic)", "Credit card (automatic)"])
});

type FormValues = z.infer<typeof formSchema>;

export default function CustomerForm({ onResult }: { onResult: (result: PredictionResult) => void }) {
  ...
  return (
    <Card className="border-border shadow-md">
      <CardHeader className="border-b border-border/50 bg-muted/20 pb-4">
        ...
        <CardDescription>Adjust the parameters below to test the model's sensitivity.</CardDescription>
      </CardHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-sm text-primary mb-2">Account Details</h3>
                ...
                <FormField control={form.control} name="paymentMethod" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                      <SelectContent>
                        {Object.values(CustomerInputPaymentMethod).map(val => (
                          <SelectItem key={val} value={val}>{val}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              {/* Services */}
              <div className="space-y-4">
                <h3 className="font-semibold text-sm text-primary mb-2">Services</h3>
                <FormField control={form.control} name="internetService" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                      <SelectContent>
                        {Object.values(CustomerInputInternetService).map(val => (
                          <SelectItem key={val} value={val}>{val}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <FormField control={form.control} name="multipleLines" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                      <SelectContent>
                        {Object.values(CustomerInputMultipleLines).map(val => (
                          <SelectItem key={val} value={val}>{val}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <FormField control={form.control} name="onlineSecurity" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                      <SelectContent>
                        {Object.values(CustomerInputOnlineSecurity).map(val => (
                          <SelectItem key={val} value={val}>{val}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <FormField control={form.control} name="techSupport" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                      <SelectContent>
                        {Object.values(CustomerInputTechSupport).map(val => (
                          <SelectItem key={val} value={val}>{val}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <FormField control={form.control} name="deviceProtection" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                      <SelectContent>
                        {Object.values(CustomerInputDeviceProtection).map(val => (
                          <SelectItem key={val} value={val}>{val}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              {/* Demographics & Media */}
              <div className="space-y-4">
                <h3 className="font-semibold text-sm text-primary mb-2">Demographics & Media</h3>
                ...
                <FormField control={form.control} name="streamingTV" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                      <SelectContent>
                        {Object.values(CustomerInputStreamingTV).map(val => (
                          <SelectItem key={val} value={val}>{val}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <FormField control={form.control} name="streamingMovies" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl>
                      <SelectContent>
                        {Object.values(CustomerInputStreamingMovies).map(val => (
                          <SelectItem key={val} value={val}>{val}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
            ...
            <div className="pt-4 flex justify-end">
              <Button type="submit" size="lg" disabled={predictMutation.isPending} className="px-8 font-semibold">
